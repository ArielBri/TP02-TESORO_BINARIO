...
TesoroBinario::TesoroBinario() {
    ...
    this->crearVectorDeJugadores(3);
    this->jugadorActual = NULL;
    this->mazo = new Mazo();
    this->turnoValido = Valido;
    this->consola = new Consola;
    this->bitmap = new Bitmap;
    this->cantidadDeJugadores= this->vectorDeJugadores->getLongitud();
}
...
void TesoroBinario::inicializarTesoros() {
	for(int i=1;i<=this->vectorDeJugadores->getLongitud();i++){
		Coordenada* coordenada= new Coordenada(1,1,i);
		Ficha* tesoro= new Ficha(Tesoro,i,coordenada);
		this->vectorDeJugadores->obtener(i)->obtenerFichasDelJugador()->add(tesoro);
	}
}
...
void TesoroBinario::jugarBlindaje() {
    ...
    while (fichas->avanzarCursor()){
        ...
        if (ficha->getTipo()==Tesoro){
            ficha->blindarTesoro();
        	this->consola->imprimirTexto("Se blindo un tesoro");
            return;
        }
    }
...
void TesoroBinario::jugarRadar() {
    ...
    this->consola->imprimirTexto("Se van mostrar las coordenadas de tesoros para el radar puesto");
    for(int X = Xi; X<=Xf; X++){
        for(int Y = Yi; Y<=Yf; Y++){
            for(int Z = Zi; Z<=Zf; Z++){
        ...
        for(int y=1; y<=alto; y++){
            for(int z=1; z<=largo; z++){
                ...
                if(casillero->getEstado()==Vacio){
                    ...
                    this->consola->imprimirTexto("Se partio un tesoro");
                    return;
                }
            }
        }
            ...
            if(casillero->getEstado()==Ocupado){
                switch (casillero->getFicha()->getTipo()) {
                    ...
                    case Espia:
                        ...
                        delete casillero->getFicha();
                        casillero->setFicha(NULL);
                        this->consola->imprimirTexto("El espia se encontro con otro espia y se eliminaron ambos");
                        break;
                    case TesoroMina:
                        this->eliminarFichaDeLaListaDeFichasDelJugador(casillero->getFicha());
                        casillero->setEstado(Inactivo);
                ...
                this->consola->imprimirTexto("Se puso el espia en la coordenada dada");
            }
        } else {
            this->consola->imprimirTexto("El espia murio porque el casillero estaba inactivo");
        }
    }
