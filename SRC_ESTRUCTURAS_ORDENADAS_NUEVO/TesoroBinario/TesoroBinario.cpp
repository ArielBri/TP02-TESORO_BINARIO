            ...
            for(int Z = Zi; Z<=Zf; Z++){
                if(this->hayTesoroEnCasillero(X,Y,Z)){
                    this->consola->imprimirTexto(
                            "Hay un tesoro en la coordenada: ("+std::to_string(X)+","+std::to_string(Y)+","+std::to_string(Z)+")");
                }
            }
        }
...
void TesoroBinario::jugarObtenerCartas4() {
    int contador = 0;
    std::string contadorString;
    for(int i=0; i<rand()%4; i++){
        this->jugadorActual->obtenerCarta(this->mazo);
        contador++;
    }
    contadorString = std::to_string(contador);

    this->consola->imprimirTexto("El numero de cartas que se agregaron es: "+contadorString);
}
            ...
            case Ocupado:
                ...
                if (casilleroActual->getEstado() == Ocupado && casilleroActual->getFicha()->getTipo()==Tesoro){
                    ...
                    switch (fichaCasilleroNuevo->getTipo()) {
                        case Tesoro:
                            this->consola->imprimirTexto("Hay un Tesoro en la coordenada: ("+
                            std::to_string(coordenadaNueva->getAncho())+","+
                            std::to_string(coordenadaNueva->getAlto())+","+
                            std::to_string(coordenadaNueva->getLargo())+")");
                            delete coordenadaNueva;
                        ...
                        case TesoroMina:
                            ...
                            break;
                        case TesoroBlindado:
                            this->consola->imprimirTexto("Hay un Tesoro Blindado en la coordenada: ("+
                                                         std::to_string(coordenadaNueva->getAncho())+","+
                                                         std::to_string(coordenadaNueva->getAlto())+","+
                                                         std::to_string(coordenadaNueva->getLargo())+")");
                            delete coordenadaNueva;
